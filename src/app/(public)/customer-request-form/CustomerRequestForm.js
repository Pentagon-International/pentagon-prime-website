...
import { imoClass } from "@/app/utils/imoClass";
import {
  contSize,
  getContainerFields,
  options,
...
import { getUnitCode } from "@/app/utils/containerUnitCodes";
...
const CustomerRequestForm = (
  ...
  // Container sizes shown only for the selected container type (isShow)
  const filteredContSize = useMemo(
    () =>
      contSize.filter(
        (s) =>
          s.isShow &&
          Array.isArray(s.isShow) &&
          s.isShow.includes(containerList.type),
      ),
    [containerList.type],
  );
  ...
  const handleSubmit = async (values) => {
    ...
    // Tariff check: call API first; if tariff data exists, show results page; else submit quote
    ...
    try {
      ...
      const tariffData = await tariffRes.json().catch(() => null);

      if (
        tariffData &&
        typeof tariffData.count === "number" &&
        tariffData.count >= 1 &&
      ...
      console.error("Tariff check error:", err);
    } finally {
      setIsTariffChecking(false);
    }
      ...
      >
        ...
        <Modal
          ...
          styles={{
            ...
            header: { color: "#fff", background: "#4c4f56" },
            title: { fontSize: "20px", fontWeight: 700, color: "#fff" },
            body: { color: "rgba(255,255,255,.9)", padding: "16px 24px" },
            close: {
              color: "#1c7ed6",
            },
          }}
