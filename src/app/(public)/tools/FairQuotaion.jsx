...
// import { apiCallProtected } from "@/lib/api/api";
...
import React, { useMemo, useState } from "react";
import { COLORS } from "@/app/utils/COLORS";
import { IconPlane, IconSquareHalf, IconBox } from "@tabler/icons-react";
import { useFormik } from "formik";
import { useDisclosure } from "@mantine/hooks";
import { useQuery } from "@tanstack/react-query";
import { apiCallProtected } from "@/lib/api/api";
...
const FairQuotation = ({ onSubmit, formHook, accordion, setAccordion }) => {
  ...
  const filteredDestinationOptions = useMemo(() => {
    if (!values.originPort) return memoizedTransportData;
    return memoizedTransportData?.filter(
      (item) => item.value !== values.originPort
    ) || [];
  }, [memoizedTransportData, values.originPort]);

  // Filter origin options to exclude the selected destination
  const filteredOriginOptions = useMemo(() => {
    if (!values.destinationPort) return memoizedTransportData;
    return memoizedTransportData?.filter(
      (item) => item.value !== values.destinationPort
    ) || [];
  }, [memoizedTransportData, values.destinationPort]);

  const handleClearOrigin = () => {
    setFieldValue("origin", "");
    setFieldValue("originPort", "");
    setFieldValue("originCode", "");
    setFieldValue("originCountry", "");
    setFieldValue("originCountryCode", "");
  };

  const handleClearDestination = () => {
    setFieldValue("destination", "");
    setFieldValue("destinationPort", "");
    setFieldValue("destinationCode", "");
    ...
    <Box style={{ maxHeight: "100vh" }}>
      <form onSubmit={handleSubmit}>
        <Grid>
          <Grid.Col span={12}>
            ...
            <SegmentedControl
              ...
              onChange={(val) => {
                ...
                setFieldValue("typeOfBooking", val);
                // Reset port selections when transport type changes
                handleClearOrigin();
          ...
          <Grid.Col span={6}>
            <Select
              ...
              onChange={(v, opt) => {
                ...
                setFieldValue("origin", opt.label);
                setFieldValue("originCountry", opt?.country_id || opt?.Country?.id);
                setFieldValue("originPort", v);
                setFieldValue("originCode", opt.code);
                setFieldValue("originCountryCode", opt.country_code);
              }}
          ...
          <Grid.Col span={12}>
            <FairContainer type={values.typeOfBooking}
              data={values?.containers}
              submitCallback={addContainerCallback}
            />
          </Grid.Col>
