...
Pentagon Freight is a web-based logistics management platform designed to streamline freight operations, track shipments in real-time, and manage logistics documentation efficiently. It provides a user-friendly interface for shipping and receiving, enabling efficient management of these processes.
...
```bash
# Step 1: Remove the .next build folder
npm rm -rf .next
...
- Only the **Contentful Access Token** is required for frontend content updates.
- **Never expose the Management Access Token** to the public.
- Store all tokens securely using environment variables.
...
Ensure `.env.local` is listed in `.gitignore` to prevent exposing sensitive information.
...
3. **Ensure ****`.env.local`**** is listed in ****`.gitignore`**.
